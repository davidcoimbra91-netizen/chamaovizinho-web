        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <Link href="/dashboard/novo-pedido"
              style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: '#C85A1A', color: '#fff', textDecoration: 'none', padding: '8px 16px', borderRadius: 10, fontSize: 15, fontWeight: 600 }}>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        {pedidos && pedidos.length > 0 ? (
                ...
                <div key={pedido.id} style={{ background: '#fff', border: '0.5px solid #EDE6DC', borderRadius: 14, padding: '16px 18px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                      ...
                      <span style={{ background: cat.bg, color: cat.color, borderRadius: 99, padding: '2px 10px', fontSize: 14, fontWeight: 600 }}>{cat.label}</span>
                      <span style={{
                        ...
                        borderRadius: 99, padding: '2px 10px', fontSize: 14,
                      }}>
                        {pedido.status === 'open' ? 'em aberto' : pedido.status === 'completed' ? 'concluído' : pedido.status}
                      </span>
                      {pedido.is_archived && <span style={{ background: '#F0EDE8', color: '#9B7A5A', borderRadius: 99, padding: '2px 10px', fontSize: 14 }}>arquivado</span>}
                    </div>
                    <span style={{ fontSize: 13, color: '#B09070' }}>{new Date(pedido.created_at).toLocaleDateString('pt-PT')}</span>
                  </div>
                  <p style={{ fontSize: 15, fontWeight: 600, color: '#2C1A0E', marginBottom: 6 }}>{pedido.title}</p>
                  {pedido.city && <p style={{ fontSize: 14, color: '#7A6048', marginBottom: 10 }}>📍 {pedido.city}{pedido.budget > 0 ? ` · € ${pedido.budget}` : ''}</p>}
          ...
          <div style={{ background: '#fff', border: '0.5px solid #EDE6DC', borderRadius: 14, padding: '48px 20px', textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 16, fontWeight: 600, color: '#2C1A0E', marginBottom: 6 }}>Ainda não tens pedidos</p>
            <p style={{ fontSize: 15, color: '#9B7A5A', marginBottom: 16 }}>Publica o teu primeiro pedido gratuitamente.</p>
