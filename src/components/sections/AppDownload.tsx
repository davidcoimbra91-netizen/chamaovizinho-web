  ...
  return (
    <section style={{ background: '#2C1A0E', padding: '48px 0' }}>
      <div className="max-w-2xl mx-auto px-4 text-center">
        <p style={{ fontSize: 11, color: '#C85A1A', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10 }}>Disponível no teu telemóvel</p>
          ...
          {[
            ...
            <a key={btn.store} href={btn.href} target="_blank" rel="noopener noreferrer"
              ...
              <div style={{ textAlign: 'left' }}>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)' }}>{btn.sub}</div>
                <div style={{ fontSize: 14, fontWeight: 500, color: '#FAF7F2' }}>{btn.store}</div>
              </div>
            </a>
          ))}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginTop: 20 }}>
          <span style={{ color: '#C85A1A', fontSize: 14 }}>★★★★★</span>
          <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.3)' }}>4.9 · Mais de 500 avaliações</span>
        </div>
      </div>
    </section>
  )
