...
import { Save, Upload, CheckCircle, Building2, CreditCard, FileText, Settings2 } from 'lucide-react'
  ...
  return (
    <div style={{ background: '#fff', border: '0.5px solid #EDE6DC', borderRadius: 14, overflow: 'hidden' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 16px', borderBottom: '0.5px solid #EDE6DC', background: '#FAF7F2' }}>
        ...
        <p style={{ fontSize: 13, fontWeight: 700, color: '#5A3E28', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>{title}</p>
      </div>
...
export default function DefinicoesFaturacaoTab() {
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    } catch (e: any) { setError('Erro ao carregar logo: ' + (e.message ?? '')) }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ fontFamily: 'Lora, serif', fontSize: 26, fontWeight: 700, color: '#2C1A0E', marginBottom: 4 }}>Perfil de Faturação</h1>
          <p style={{ fontSize: 14, color: '#7A6048' }}>Dados que aparecem nos teus documentos PDF</p>
        </div>
        <button onClick={handleSave} disabled={saving || uploadingLogo}
          style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '9px 18px', borderRadius: 99, background: saved ? '#3B6D11' : '#C85A1A', border: 'none', color: '#fff', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
      ...
      <Section icon={Upload} title="Logo da empresa">
        ...
        {logoUrl ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <img src={logoUrl} alt="Logo" style={{ height: 56, maxWidth: 180, objectFit: 'contain', borderRadius: 8, border: '0.5px solid #EDE6DC', background: '#fff', padding: 6 }} />
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              ...
              <button onClick={() => fileInputRef.current?.click()} disabled={uploadingLogo}
                style={{ fontSize: 13, color: '#C85A1A', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer', padding: 0, textAlign: 'left' as const }}>
                {uploadingLogo ? 'A carregar...' : 'Alterar logo'}
              </button>
              <button onClick={() => setLogoUrl('')}
                style={{ fontSize: 13, color: '#9B7A5A', background: 'none', border: 'none', cursor: 'pointer', padding: 0, textAlign: 'left' as const }}>
                Remover
              </button>
            </div>
          </div>
          ...
          <button onClick={() => fileInputRef.current?.click()} disabled={uploadingLogo}
            style={{ width: '100%', padding: '18px', borderRadius: 10, border: '1.5px dashed #C85A1A', background: '#FBF0E8', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
            <Upload size={22} color="#C85A1A" />
            <p style={{ fontSize: 14, fontWeight: 600, color: '#C85A1A', margin: 0 }}>{uploadingLogo ? 'A carregar...' : 'Carregar logo'}</p>
      ...
      <Section icon={FileText} title="Regime de IVA">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          {VAT_OPTIONS.map(opt => (
            <button key={opt.value} onClick={() => setVatRegime(opt.value)}
              style={{ padding: '12px 14px', borderRadius: 10, textAlign: 'left' as const, cursor: 'pointer', border: vatRegime === opt.value ? '1.5px solid #C85A1A' : '0.5px solid #EDE6DC', background: vatRegime === opt.value ? '#FBF0E8' : '#fff' }}>
      ...
      <Section icon={FileText} title="Notas de rodapé padrão">
        <Field label="Texto do rodapé" hint="Aparecerá no fundo de todos os documentos PDF gerados.">
          <textarea style={{ ...inputStyle, minHeight: 90, resize: 'vertical' as const, fontFamily: 'inherit' }}
            value={defaultFooter} onChange={e => setDefaultFooter(e.target.value)}
            placeholder="Ex: Obrigado pela preferência. Pagamento a 30 dias por transferência bancária." />
        </Field>
      </Section>

      <button onClick={handleSave} disabled={saving || uploadingLogo}
        style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '12px 24px', borderRadius: 12, background: saved ? '#3B6D11' : '#C85A1A', border: 'none', color: '#fff', fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>
