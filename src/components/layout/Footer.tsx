    ...
    <footer style={{ background: '#2C1A0E', color: '#FAF7F2' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10">
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              ...
              <span style={{ fontFamily: 'Lora, serif', fontSize: 16, fontWeight: 600 }}>Chama o Vizinho!</span>
            </div>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
              {['App Store', 'Google Play'].map(s => (
                <span key={s} style={{ background: 'rgba(255,255,255,0.08)', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: '5px 10px', fontSize: 13, cursor: 'pointer', color: 'rgba(255,255,255,0.6)' }}>{s}</span>
              ))}
            </div>
          </div>
          ...
          {[
            ...
            <div key={col.title}>
              <h4 style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 12 }}>{col.title}</h4>
              <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 8 }}>
                {col.links.map(link => (
                  <li key={link.href}>
                    <Link href={link.href} style={{ fontSize: 15, color: 'rgba(255,255,255,0.4)', textDecoration: 'none' }} className="hover:text-white transition-colors">{link.l}</Link>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div style={{ borderTop: '0.5px solid rgba(255,255,255,0.08)', marginTop: 40, paddingTop: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
          <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.2)' }}>© {new Date().getFullYear()} Chama o Vizinho. Todos os direitos reservados.</p>
          <div style={{ display: 'flex', gap: 16 }}>
            {[{ href: '/politica-privacidade', l: 'Privacidade' }, { href: '/termos-condicoes', l: 'Termos' }].map(link => (
              <Link key={link.href} href={link.href} style={{ fontSize: 14, color: 'rgba(255,255,255,0.2)', textDecoration: 'none' }} className="hover:text-white/50 transition-colors">{link.l}</Link>
            ))}
          </div>
        </div>
      </div>
